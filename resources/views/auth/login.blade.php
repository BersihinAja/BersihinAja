<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login') }}" class="mt-8 space-y-5">
        ...
        <div class="flex items-center justify-between">
            ...
            @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}" class="text-xs font-bold text-mint ease-premium hover:text-purple">Lupa password?</a>
            @endif
        </div>
    ...
    <p class="mt-8 text-center text-sm font-medium text-charcoal/50">
        Belum punya akun?
        <a href="{{ route('register') }}" class="font-black text-mint ease-premium hover:text-purple">Daftar sekarang</a>
    </p>
</x-guest-layout>
